...
def main(target_domain):
    try:
        ...
        print(f"[!] {target_domain} page could not be accessed: {e}")
        return
    ...
    print(f"[+] {len(subdomains)} external source found.")

    for subd in sorted(subdomains):
        print(f"[*] Checking: {subd}")
        vulnerable, info = check_takeover(subd)
        if vulnerable:
            print(f"\033[91m[!] POSSIBLE TAKEOVER: {subd} -> CNAME {info[0]} (signature: '{info[1]}')\033[0m")
        else:
            print(f"[-] SAFE: {subd}")

if __name__ == '__main__':
    domains = ['example.com','example2.com'] #Write domains
